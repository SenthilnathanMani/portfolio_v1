...
export default function Navbar() {
  ...
  return (
    <motion.header style={{ backgroundColor: background }} className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-transparent ${hasScrolled ? 'shadow-md border-gray' : 'shadow-none'}`}>
      <div className="mx-auto py-4 w-full">
        <nav className="flex items-center justify-evenly">
          <motion.div className="text-white font-bold text-xl " initial={{opacity: 0,  y: -20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <motion.div className="space-x-8 text-white font-medium hidden md:flex" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
